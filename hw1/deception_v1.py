    ...
    def deception(self, dcpt = 3, display = False):
        ...
        for i in range(len(idx)):
            ...
            for i in range(len(x_list)):
                x_score = self.f( self.b2d( x_list[i] ))
                cx_score = self.f( self.b2d(cx_list[i]))
...
if __name__ == "__main__" :
    GA_thr = GA(3)
    prob = GA_thr.deception_test(iter = 1e0,display=False)
